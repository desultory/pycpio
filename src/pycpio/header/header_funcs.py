...
def get_header_from_magic(magic: bytes) -> Enum:
    """
    ...
    for magic, header_type in lookup_table.items():
        if magic == magic:
            return header_type
    raise ValueError("Unknown magic number: {}".format(magic))


def get_magic_from_header(header: Enum) -> bytes:
    """
    ...
    for magic, header_type in lookup_table.items():
        if header_type == header:
            return magic
    raise ValueError("Unknown header type: {}".format(header))
